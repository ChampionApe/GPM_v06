...
def standardArgs(settings, db, gdx, blocks = None, functions = None, run = True, prefix = '', prefix_run = '', options = {}):
	""" Returns the 'standard' components. Use GmsPy.Groups to handle grouping of variables. """
	args = {'root': write_root(**options), prefix+'functions': args_functions(f=functions), prefix+'declare': write_declare(db, gdx, from_db=settings.from_db), 
			prefix+'blocks': blocks}
	args = {k:v for k,v in args.items() if v}
	return args | standardRun(settings,db,prefix=prefix_run) if run else args

def standardArgs_gamY(settings,db,gdx, blocks = None, functions = None, declare_groups = '', run = True, prefix = '', prefix_run='', options={}):
	args = {'root': write_root(**options), prefix+'functions': args_functions(f=functions), prefix+'declare': write_declare(db, gdx, from_db='sets'), 

...
def robust_merge_dbs(db1,db2,priority='second'):
	...
	if isinstance(db1,(GpyDB,SeriesDB,dict)):
		if isinstance(db2,(GpyDB,SeriesDB,dict)):
			...
			merge_dbs_GpyDB_gams(db1,db2,priority=priority)
	elif isinstance(db1,gams.GamsDatabase):
		if isinstance(db2,(GpyDB,SeriesDB,dict)):
			merge_dbs_gams_GpyDB(db1,db2,priority=priority)
		elif isinstance(db2,gams.GamsDatabase):
			merge_dbs_gams(db1,db2,priority=priority)

def merge_dbs_GpyDB(db1,db2,priority='second'):
	"""" merge db2 into db1. """
	if priority == 'second':
		[GpyDBs_AOM_Second(db1,symbol) for symbol in db2];
	elif priority== 'first':
		[GpyDBs_AOM_First(db1,symbol) for symbol in db2];
...
def merge_dbs_gams_GpyDB(db_gms,db_py,g2np,priority='second'):
	...
	if priority == 'second':
		[gpy2db_gams_AOM(s,db_gms,g2np,merge=True) for s in db_py];
	elif priority == 'first':
...
def merge_dbs_gams(db1,db2,g2np,priority='second'):
	...
	if priority=='second':
		...
		merge_dbs_GpyDB_gams(d,db1) # merge into d with priority to db1.
...
def robust_add_or_merge(db,symbol,db_from=None,g2np=None,merge=False,**kwargs):
	...
	if isinstance(db,(dict, GpyDB,SeriesDB)):
		...
		gpy2db_gams_AOM(s,db,g2np,merge=True)
	else:
		raise TypeError("Check type(db).")

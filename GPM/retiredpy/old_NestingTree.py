...
from DBWheels_mi import  mergeMultiIndex
...
class AggTree:
	...
	def attrs_from_trees(self):
		...
		self['map'] = mergeMultiIndex([tree.get('map') for tree in self.trees.values()])
		self['branch'] = mergeMultiIndex([tree.get('branch') for tree in self.trees.values()])
		self['knot'] = mergeMultiIndex([tree.get('knot') for tree in self.trees.values()])
